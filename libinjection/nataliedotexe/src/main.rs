...
use windows::{
        ...
        Win32::
        {
            System::
            {
                LibraryLoader::{ LoadLibraryW, GetModuleHandleW, GetProcAddress},
                Diagnostics::Debug::{WriteProcessMemory, FORMAT_MESSAGE_FROM_SYSTEM, FORMAT_MESSAGE_IGNORE_INSERTS, FormatMessageW}, 
                Memory::{
                    MEM_COMMIT, 
...
unsafe fn do_crimes(args: Args) -> Result<(), Report> {
    ...
    let kernel32 = GetModuleHandleW(
        PCWSTR(u16cstr!("kernel32.dll").as_ptr()))?;
    ...
    .ok_or(eyre!("LoadLibraryW not found!"))? as *const ();
